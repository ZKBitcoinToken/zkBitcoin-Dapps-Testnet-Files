<template>
  <div id="app" v-if="!mainLoading">
    <Header />
<!---
<h1 style="text-align: center;">Enter Ethereum Address</h1>
    <input class="existing-input" v-model="existingInput" placeholder="Your existing input">

    ---><!-- Ethereum Address Text Box -->
  <!---  <input class="ethereum-address-input" v-model="ethereumAddress" :style="{ width: textBoxWidth }" :placeholder="userAccountsss">
   --->
...
<a :href="'https://goerli.explorer.zksync.io/address/' + userAccountsss">
    {{ userAccountsss }}
        ...
        <p>
         Your zk Sync Era  ETH: <span>{{ currentBalance }}  <img src="https://zkbitcoin.org/img/ethereum.png" alt="ethereum" style="width: 25px; height: 25px;" /> {{ selectedToken.symbol }}</span>
        </p>
      </div> 
    <h1> <img src="https://zkbitcoin.org/img/zkBitcoin.png" alt="ethereum" style="width: 45px; height: 45px;" />  zkBitcoin Auctions Dapp, v1.1.3</h1>
...
<h1>LP Tokens Total Supply {{ (parseInt(greetingLPTotal)/1e18)}} LP Tokens</h1>
  ...
  <h2>Pool is worth {{ (parseInt(ReservesTotal2)/1e18) }} and {{  (parseInt(ReservesTotal)/1e18) }} </h2>
...
--->
    ...
    <h2>Current Auction Total: {{ parseFloat(currentAuctionTotal / 1e18).toFixed(18).replace(/(\.0+|(?:(\.\d+?)0+)(?=\D|$))$/, '$2') }} <img src="https://zkbitcoin.org/img/ethereum.png" alt="ethereum" style="width: 25px; height: 25px;" /> Ethereum</h2>
    <h2>Current Auction Your Total: {{ parseFloat(UserTotal / 1e18).toFixed(18).replace(/(\.0+|(?:(\.\d+?)0+)(?=\D|$))$/, '$2') }} <img src="https://zkbitcoin.org/img/ethereum.png" alt="ethereum" style="width: 25px; height: 25px;" /> Ethereum</h2>
    ...
    <h2>Time Left in Auction: {{ (NextDayTime) }} </h2>
    <h2>Auction % Remaining: {{nextDayHoursTime}}</h2>
    <h3>With your current bids you will recieve up to {{ (!isNaN(UserTotal / currentAuctionTotal) ? (UserTotal / currentAuctionTotal * 32768).toFixed(3) : 0) }}  <img src="https://zkbitcoin.org/img/zkBitcoin.png" alt="ethereum" style="width: 25px; height: 25px;" /> zkBitcoin, if no other bids are cast and the auction ends with a dust bid.</h3>
   <h3>If you bid {{number2}}  <img src="https://zkbitcoin.org/img/ethereum.png" alt="ethereum" style="width: 25px; height: 25px;" />Ethereum you will recieves up to {{ (( (parseFloat(number2)+parseFloat(UserTotal)/1e18)) / (parseFloat(currentAuctionTotal)/1e18+(parseFloat(number2))) * 32768).toFixed(5) }}  <img src="https://zkbitcoin.org/img/zkBitcoin.png" alt="zkbtc" style="width: 25px; height: 25px;" />zkBitcoin, if you bid {{ number2 }}  <img src="https://zkbitcoin.org/img/ethereum.png" alt="ethereum" style="width: 25px; height: 25px;" />Ethereum below and if the auction ends with a dust bid.</h3>
    <div>
      This a simple dApp, which you can choose the amount of ETH you want to bid at auction for  <img src="https://zkbitcoin.org/img/zkBitcoin.png" alt="ethereum" style="width: 25px; height: 25px;" />ZKBitcoin Tokens.
    </div>
    Input <img src="https://zkbitcoin.org/img/ethereum.png" alt="ethereum" style="width: 25px; height: 25px;" /> ETH to BID: <input class="form-control" type="number" id="number2" v-model="number2" placeholder="0.01" :step="StepEthToSend" :max="maxQuantity" min=0>
    <h1>Total <img src="https://zkbitcoin.org/img/ethereum.png" alt="ethereum" style="width: 25px; height: 25px;" />ETH TO SEND Contract is: {{ message }}  <img src="https://zkbitcoin.org/img/ethereum.png" alt="ethereum" style="width: 25px; height: 25px;" /> ETH</h1>
...
<a :href="'https://goerli.explorer.zksync.io/address/' + userAccountsss">
    {{ userAccountsss }}
      ...
      <button
        ...
        >
          <span v-if="selectedToken && !txStatus">Bid {{ number2 }}  <img src="https://zkbitcoin.org/img/ethereum.png" alt="ethereum" style="width: 25px; height: 25px;" /> ETH on the Current Auction</span>
          <span v-else-if="!selectedToken">Select token to pay fee first</span>
...
<h2>You can claim: {{ CLAIM_AMOUNT/1e18 }}  <img src="https://zkbitcoin.org/img/zkBitcoin.png" alt="ethereum" style="width: 25px; height: 25px;" />zkBitcoin Tokens from auctions</h2>
<!--- <h2>You can {{ GreetingDaysToWithdrawArray }} get GreetingDaysToWithdrawArray to be the array needed for the withdrawlDays abi then</h2> -->
      <button
        ...
        >
          <span v-if="selectedToken && !txStatus">Claim  <img src="https://zkbitcoin.org/img/zkBitcoin.png" alt="ethereum" style="width: 25px; height: 25px;" />zkBitcoin from Auctions</span>
          <span v-else-if="!selectedToken">Select token to pay fee first</span>
          <span v-else-if="txStatus == 1">Sending tx...</span>
          <span v-else-if="txStatus == 2"
            >Waiting until tx is committed...</span
...
<br>
 <label for="amountOfDays">Ethereum Address to bid for:</label>
      <!-- Ethereum Address Text Box -->
    <input class="ethereum-address-input" v-model="ethereumAddress" :style="{ width: textBoxWidth }" :placeholder="userAccountsss">
 
  </div>

 <h1>Sending {{ number2 }} <img src="https://zkbitcoin.org/img/ethereum.png" alt="ethereum" style="width: 25px; height: 25px;" />  ETH to bid for {{ amountOfDays }}  {{ headerText }} for {{ethereumAddress.slice(0,6) }}...{{ethereumAddress.slice(-4) }}</h1>

<h3>Bid {{  parseFloat( (number2/amountOfDays) ).toFixed(18).replace(/(\.0+|(?:(\.\d+?)0+)(?=\D|$))$/, '$2') }} ETH on each of the {{ amountOfDays }} auctions in Era #{{era}} for a total of {{ parseFloat( number2 ).toFixed(18).replace(/(\.0+|(?:(\.\d+?)0+)(?=\D|$))$/, '$2') }}  <img src="https://zkbitcoin.org/img/ethereum.png" alt="ethereum" style="width: 25px; height: 25px;" />ETH</h3>
<h3>The current Era # is {{ currentEra }} and the current Auction # is {{ greetingAuctionNumber }}</h3>
Input Total  <img src="https://zkbitcoin.org/img/ethereum.png" alt="ethereum" style="width: 25px; height: 25px;" /> ETH to Spend: <input
  class="form-control"
  type="number"
  id="number2"
  v-model="number2"
...
<br>
    ...
    <button
          class="my-button"
          :disabled="!selectedToken || txStatus != 0"
          v-on:click="sendETHtoAuctionAdvancedAddy"
        >
          <span v-if="selectedToken && !txStatus">Advanced Auction: Sending {{ number2 }} <img src="https://zkbitcoin.org/img/ethereum.png" alt="ethereum" style="width: 25px; height: 25px;" />ETH to bid for {{ amountOfDays }}  {{headerText }} for {{ethereumAddress.slice(0,6) }}...{{ethereumAddress.slice(-4) }}</span>
          <span v-else-if="!selectedToken">Select token to pay fee first</span>
          <span v-else-if="txStatus == 1">Sending tx... Sending {{ number2 }} <img src="https://zkbitcoin.org/img/ethereum.png" alt="ethereum" style="width: 25px; height: 25px;" />  ETH to bid for {{ amountOfDays }} {{ headerText }} for {{ethereumAddress.slice(0,6) }}...{{ethereumAddress.slice(-4) }}</span>
          <span v-else-if="txStatus == 2"
            >Waiting until tx is committed...</span
          >
          <span v-else-if="txStatus == 3">Updating the page...</span>
...
<h1> Advanced Claiming - Claim for other accounts </h1>

 <label for="amountOfDays">Ethereum Address to claim for:</label>
      <!-- Ethereum Address Text Box -->
    <input class="ethereum-address-input" v-model="ethereumAddress2" :style="{ width: textBoxWidth }" :placeholder="userAccountsss">
 
<h2> {{ethereumAddress2.slice(0,6) }}...{{ethereumAddress2.slice(-4) }} can claim: {{ CLAIM_AMOUNTAdvanced/1e18 }}  <img src="https://zkbitcoin.org/img/zkBitcoin.png" alt="ethereum" style="width: 25px; height: 25px;" />zkBitcoin Tokens from auctions</h2>
<!--- <h2>You can {{ GreetingDaysToWithdrawArray }} get GreetingDaysToWithdrawArray to be the array needed for the withdrawlDays abi then</h2> -->
      <button
          class="my-button"
          :disabled="!selectedToken || txStatus != 0"
          v-on:click="sendETHtoAuction2Addy"
        >
          <span v-if="selectedToken && !txStatus">Claim  <img src="https://zkbitcoin.org/img/zkBitcoin.png" alt="ethereum" style="width: 25px; height: 25px;" />2zkBitcoin from Auctions for  {{ethereumAddress2.slice(0,6) }}...{{ethereumAddress2.slice(-4) }}</span>
          <span v-else-if="!selectedToken">Select token to pay fee first</span>
          <span v-else-if="txStatus == 1">Sending tx...</span>
          <span v-else-if="txStatus == 2"
            >Waiting until tx is committed...</span
  ...
  computed: {
    shouldDisplayButton1() {
      ...
      if (parseInt(this.amountOfDays) == 1) {
        return `auction. For Era #${this.era} Auction #${startingDay}`;
      } else {
        return `auctions. For Era #${this.era} Auctions #${startingDay} - #${endDay}`;
      }
    },
  },
  watch: {
...
const myTimeout = setTimeout(() => {
  // Your timeout logic here
      ...
      if (window.ethereum.networkVersion == 280 ) {
          console.log("NEW VALUE this.loopId123123", this.loopId);
            this.loadMainScreen();
           // Start the loop
          console.log("NEW VALUE this.loopId123123", this.loopId);
...
console.log("userAddy getReserves", parseInt(getReserves222[1]) );
    ...
    async getAuctionTotalZKBTCAdvanced() {
      const userAddy = await this.getAddress()
try{
console.log("userAddy this.ethereumAddress22", this.ethereumAddress2 );
    if(this.ethereumAddress2 == "0x"){
        return await this.contract2.Check_Withdraw_Amt(userAddy);
    }else{
        return await this.contract2.Check_Withdraw_Amt(this.ethereumAddress2);
    }
...
var redret = 0;	
if(remainTime>0){
 this.nextDayHoursTime = ((100* (remainTime / 3600)/(this.AuctionLengthInHours/(60*60)))).toFixed(2).toString()+ " %"
 redret =  (remainTime / 3600).toFixed(2).toString()+" Hours";
}else{
this.nextDayHoursTime = "AUCTION time has ended. One last bid will close the auction and a new auction will begin."

 redret =  "AUCTION time has ended. One last bid will close the auction and a new auction will begin."
}
...
console.log("TME2 ", (now / 1000).toFixed(0) );
  ...
  if(this.number2 == 0.000000000041416682){
...
console.log("222212", userAddy );
    ...
    async sendETHtoAuctionAdvanced() {
      ...
      const amount = this.message;
      const amountInWei = web3.utils.toWei(parseFloat(amount).toFixed(18).toString());
      const userAddy = this.ethereumAddress2;
console.log("222212", userAddy );
    this.txStatus = 1;
    try {
    ...
    async sendETHtoAuctionAdvancedAddy() {
      const web3 = new Web3();
...
console.log("222212", userAddy );
    this.txStatus = 1;
    try {
        const txHandle = await this.contract2.FutureBurn0xEasier(this.era, this.StartingDay, this.amountOfDays, this.ethereumAddress,  {value: amountInWei});
...
console.log("222212111: FFF: ", this.userAccountsss );
    ...
    this.userAccountsss = address;
        ...
        this.CLAIM_AMOUNTAdvanced = await this.getAuctionTotalZKBTCAdvanced();
...
console.log("222212 checkitzl earzzz", erazzz );
    ...
    async sendETHtoAuction2Addy() {
        var StartingEra = 5;//await this.getGreeting
    try { 
	AuctionERA();
      const userAddy = this.ethereumAddress2;
       var checkitzl = await this.contract2.Withdrawl_ARRAY_WithCustomAmount(parseInt(StartingEra), userAddy, 1);//await this.getGreetingDaysToWithdrawArray();
...
console.log("222212", userAddy );
	...
	this.era = currentEra;
      ...
      this.getAuctionTotalZKBTCAdvanced().then((CLAIM_AMOUNTAdvanced) => {
        this.CLAIM_AMOUNTAdvanced = CLAIM_AMOUNTAdvanced;
        this.mainLoading2 = false;
      });
      ...
      this.getAddress().then((Addys) => {
        this.ethereumAddress = Addys;
        this.ethereumAddress2 = Addys;

        this.userAccountsssOLD = Addys;
        this.mainLoading = false;
      });
...
.existing-input {
  /* Style your existing input here */
  margin-bottom: 10px; /* Add margin for separation from the Ethereum address input */
}
...
.form-control { 
  max-width: 300px; /* Set your desired maximum width in pixels */
 
  width: 70%;
  padding: 3px; /* Optional: Add padding for better visual appearance */
  font-size: 130%; /* Adjust the width to your preference */
}
.larger-input {
