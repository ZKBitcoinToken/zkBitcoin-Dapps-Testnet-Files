<template>
  <div id="app" v-if="!mainLoading">
    ...
    <h1>Download Testnet Miner for zkBitcoin @ <a href="https://github.com/ZKBitcoinToken/zkBTC-Testnet-Mainnet-Almost">Click here to download Windows GPU and CPU Miner</a></h1> <h1>Get ZK Sync Testnet ETH @ <a href="https://faucet.triangleplatform.com/zksync/testnet?">CLICK HERE TO GO TO ETH FAUCET</a></h1>
...
<br>
        ...
        >
          <span v-if="selectedToken && !txStatus">Get Test 0xBitcoin Tokens  <img src="https://zkbitcoin.org/img/0xbitcoin.png" alt="0xBitcoin" style="width: 45px; height: 45px;" />  to test sending Rewards Contract some to test rewards</span>
...
<a :href="'https://goerli.explorer.zksync.io/address/' + userAccountsss">
    {{ userAccountsss }}
        ...
        <p>
          Your zkSync Era ETH: <span>{{ currentBalance }}<img src="https://zkbitcoin.org/img/ethereum.png" alt="ethereum" style="width: 15px; height: 15px;" /> {{ selectedToken.symbol }}</span>
        </p>
        
      </div>  
    <h1> zkBitcoin Staking 0xBitcoin  <img src="https://zkbitcoin.org/img/0xbitcoin.png" alt="0xBitcoin" style="width: 45px; height: 45px;" />  Dapp, v1.1.3</h1>
    <h2>Greeter says: {{ greeting }} 👋</h2>

<h2>Your Unstaked and Staked LP Tokens are worth a total {{ (parseInt(ReservesTotal)/1e8* (parseInt(greetingStakeBalance)+parseInt(MaxsliderValue2)) / greetingLPTotal).toFixed(8) }}  <img src="https://zkbitcoin.org/img/0xbitcoin.png" alt="0xBitcoin" style="width: 25px; height: 25px;" />0xBTC and {{  (parseInt(ReservesTotal2)/1e18* (parseInt(greetingStakeBalance)+parseInt(MaxsliderValue2)) / greetingLPTotal) }} <img src="https://zkbitcoin.org/img/zkBitcoin.png" alt="0xBitcoin" style="width: 25px; height: 25px;" />zkBTC</h2>
<h1>Depositing</h1>
...
<p>Selected slider value: {{ parseFloat(sliderValue2).toFixed(13).replace(/(\.0+|(?:(\.\d+?)0+)(?=\D|$))$/, '$2') }} </p>
	...
	<button
        ...
        >
          <span v-if="shouldDisplayButton1 && selectedToken && !txStatus"> or Click this to approve {{ parseFloat(sliderValue2).toFixed(13).replace(/(\.0+|(?:(\.\d+?)0+)(?=\D|$))$/, '$2') }} <img src="https://zkbitcoin.org/img/syncswap.png" alt="0xBitcoin" style="width: 25px; height: 25px;" /> LP Tokens to the Staking Contract in<br> 2 transactions</span>
          <span v-else-if="!selectedToken">Select token to pay fee first</span>
          <span v-else-if="txStatus == 1">Sending tx...</span>
          <span v-else-if="txStatus == 2"
            >Waiting until tx is committed...</span
 ...
 <button
        ...
        >
          <span v-if="!shouldDisplayButton1 && selectedToken && !txStatus">Step 2) Click this to deposit {{ parseFloat(sliderValue2).toFixed(13).replace(/(\.0+|(?:(\.\d+?)0+)(?=\D|$))$/, '$2')}} <img src="https://zkbitcoin.org/img/syncswap.png" alt="0xBitcoin" style="width: 25px; height: 25px;" /> LP Tokens to the Staking Contract</span>
          <span v-else-if="!selectedToken">Select token to pay fee first</span>
          <span v-else-if="txStatus == 1">Sending tx...</span>
          <span v-else-if="txStatus == 2"
            >Waiting until tx is committed...</span
...
<h2>Your Staked LP Tokens are worth {{ (parseInt(ReservesTotal)/1e8* greetingStakeBalance / greetingLPTotal).toFixed(8) }} <img src="https://zkbitcoin.org/img/0xbitcoin.png" alt="0xBitcoin" style="width: 25px; height: 25px;" /> 0xBTC and {{  (parseInt(ReservesTotal2)/1e18* greetingStakeBalance / greetingLPTotal) }} <img src="https://zkbitcoin.org/img/zkBitcoin.png" alt="0xBitcoin" style="width: 25px; height: 25px;" />zkBTC </h2>
    ...
    -->
    <h2>Your Reward {{  (StakeReward/1e18).toFixed(8) }} <img src="https://zkbitcoin.org/img/zkBitcoin.png" alt="0xBitcoin" style="width: 25px; height: 25px;" />zkBitcoin Tokens </h2>
...
<h3>Total Amount of Staked LP Tokens: {{ parseFloat(MaxsliderValue/1e18).toFixed(12).replace(/(\.0+|(?:(\.\d+?)0+)(?=\D|$))$/, '$2') }} <img src="https://zkbitcoin.org/img/syncswap.png" alt="0xBitcoin" style="width: 25px; height: 25px;" />LP Tokens</h3>
...
<h3>Selected tokens to withdraw: {{ parseFloat(sliderValue).toFixed(12).replace(/(\.0+|(?:(\.\d+?)0+)(?=\D|$))$/, '$2') }} <img src="https://zkbitcoin.org/img/syncswap.png" alt="0xBitcoin" style="width: 25px; height: 25px;" />LP Tokens</h3>
        ...
        >
          <span v-if="selectedToken && !txStatus">Click this to withdraw {{ parseFloat(sliderValue).toFixed(13).replace(/(\.0+|(?:(\.\d+?)0+)(?=\D|$))$/, '$2')}} <img src="https://zkbitcoin.org/img/syncswap.png" alt="0xBitcoin" style="width: 25px; height: 25px;" />LP Tokens</span>
          <span v-else-if="!selectedToken">Select token to pay fee first</span>
          <span v-else-if="txStatus == 1">Sending tx...</span>
          <span v-else-if="txStatus == 2"
            >Waiting until tx is committed...</span
...
<br><br>
         ...
         <button
          ...
          v-on:click="ExitOnly"
        >
          <span v-if="selectedToken && !txStatus">Remove All <img src="https://zkbitcoin.org/img/syncswap.png" alt="0xBitcoin" style="width: 25px; height: 25px;" />  SyncSwap LP Tokens Staked and Claim Rewards</span>
          <span v-else-if="!selectedToken">Select token to pay fee first</span>
          <span v-else-if="txStatus == 1">Sending tx...</span>
          <span v-else-if="txStatus == 2"
            >Waiting until tx is committed...</span
...
<h2>Advanced Depositing</h2>
<h3>Deposit YOUR LP tokens for other users</h3>
...
Adjust below to add a portion of the LP Tokens
<p>Selected slider value: {{ parseFloat(sliderValue2).toFixed(13).replace(/(\.0+|(?:(\.\d+?)0+)(?=\D|$))$/, '$2') }} </p>
    GetPermitAndStakeAdvanced  <button
          class="my-button"
          :disabled="!selectedToken || txStatus != 0"
...
<h3>Claim rewards for other users</h3>
 ...
 <button
        ...
        >
          <span v-if="selectedToken && !txStatus">Claim Staking Rewards for {{ ethereumAddress2.slice(0,6) }}...{{ ethereumAddress2.slice(-4) }}</span>
          <span v-else-if="!selectedToken">Select token to pay fee first</span>
          <span v-else-if="txStatus == 1">Sending tx...</span>
          <span v-else-if="txStatus == 2"
            >Waiting until tx is committed...</span
...
<input
  ...
  min=0
/>
<h2>Total ETH TO SEND Contract is: {{ message }} ETH</h2>
...
<!--
    <div class="balance" v-if="selectedToken">
        ...
        <p>
          ...
          <span v-else>{{ currentBalance }} <img src="https://zkbitcoin.org/img/ethereum.png" alt="ethereum" style="width: 1.5%; height: 1.5%;" />{{ selectedToken.symbol }}</span>
        </p>
        <p>
...
<br>
 ...
 <h1>{{ headerText }}</h1>
<h3>Bid {{  parseFloat( (number2/amountOfDays) ).toFixed(18).replace(/(\.0+|(?:(\.\d+?)0+)(?=\D|$))$/, '$2') }} ETH on each of the {{ amountOfDays }} auctions in Era #{{era}} for a total of {{ parseFloat( (number2 ).toFixed(18).replace(/(\.0+|(?:(\.\d+?)0+)(?=\D|$))$/, '$2') }} ETH</h3>
...
<h1>LP Tokens Total Supply {{ parseFloat( (parseInt(greetingLPTotal)/1e18)).toFixed(18).replace(/(\.0+|(?:(\.\d+?)0+)(?=\D|$))$/, '$2') }} LP Tokens</h1>
  ...
  <h2>Pool is worth {{ parseFloat( (parseInt(ReservesTotal)/1e8)).toFixed(8).replace(/(\.0+|(?:(\.\d+?)0+)(?=\D|$))$/, '$2') }} 0xBTC and {{  parseFloat( (parseInt(ReservesTotal2)/1e18)).toFixed(18).replace(/(\.0+|(?:(\.\d+?)0+)(?=\D|$))$/, '$2') }} zkBTC </h2>
  
    <h2>Total 0xBTC in Pool = {{ parseFloat( (parseInt(ReservesTotal)/1e8)).toFixed(12).replace(/(\.0+|(?:(\.\d+?)0+)(?=\D|$))$/, '$2') }} 0xBTC </h2>
...
<h1> Percent of Total Liquidity Staked for Pool {{ (greetingStakeBalanceTotal / greetingLPTotal*100 ).toFixed(4) }} % </h1>
    ...
    <h2>Your Total 0xBTC in Pool ={{ parseFloat( (parseInt(ReservesTotal)/1e8* greetingStakeBalance / greetingLPTotal) ).toFixed(8).replace(/(\.0+|(?:(\.\d+?)0+)(?=\D|$))$/, '$2')       }} 0xBTC </h2>
    ...
    <h2>Your Approved LP Tokens for Contract {{  parseFloat( ApprovedLPforContract/ 1e18 ).toFixed(12).replace(/(\.0+|(?:(\.\d+?)0+)(?=\D|$))$/, '$2') }} LP Tokens</h2>
...
const LP_CONTRACT_ADDRESS = "0xBA6d9490927F71069c676f251060913b095a5741"
const Staking_zkBTC_CONTRACT_ADDRESS = "0x5Dfa53ee580ccA7F355b626bED1b1Ed0a6383E57"
...
const LP_SECOND_CONTRACT_ADDRESS = "0xe21010cdA483ac1Ae6375e5D50dB2293307C4b60"
const LP_SECOND_STAKING_CONTRACT_ADDRESS = "0xE673c6EB8Fa1E8F1E6b09E29F473237F90C837b8"
...
console.log("222212", userAddy );
	...
	this.era = currentEra;
      ...
      this.getAddress().then((ethereumAddress) => {
        this.ethereumAddress = ethereumAddress;
        this.ethereumAddress2 = ethereumAddress;
        this.mainLoading = false;
      });
